         ...
         <form @submit.prevent="studentApply">
            ...
            <div class="alert alert-primary">
                <ul style="list-style-type: disc; list-style-position: inside;" class="text-info">
                    <li>প্রথমে আপনার বিষয়/বিভাগ সিলেক্ট করে, নিম্নের <span class="btn btn-sm text-success"><i class="fas fa-arrow-circle-up"></i></span> এবং <span class="btn btn-sm text-primary"><i class="fas fa-arrow-circle-down"></i></span> বাটনে ক্লিক করে আপনার পছন্দক্রম পরিবর্তন করুন।</li>
                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table-sm text-center w-100" style="line-height: 1; font-size: 0.9rem">
                                    <thead>
                                        <th class="" style="width: 20%">পছন্দক্রম</th>
                                        <th class="text-left">বিষয়/বিভাগ</th>                                        
                                    </thead>
                                    <tbody>
                                        <tr v-for="(dept,i) in form.choiceSubs" :key="i" @click="select_dept(i)" :class="{ 'alert-primary': i==currentSub }" style="cursor: pointer; user-select: none;">
                                            <td>{{ enToBn(i) }}</td>
                            ...
                            <div class="card-footer bg-white border d-flex justify-content-center">
                                <span class="btn btn-light text-success mr-3" style="font-size: 1.5rem;" @click="upChoice()"><i class="fas fa-arrow-circle-up"></i></span>
                                <span class="btn btn-light text-primary ml-3" style="font-size: 1.5rem;" @click="downChoice()"><i class="fas fa-arrow-circle-down"></i></span>
                            </div>
                        </div>
                    </div>
        ...
        methods: {
            ...
            upChoice() {
                if(this.currentSub>0) {
                    var element = this.form.choiceSubs[this.currentSub];
                    this.form.choiceSubs.splice(this.currentSub, 1);
                    this.currentSub--;
                    this.form.choiceSubs.splice(this.currentSub, 0, element);
                }
            },
            downChoice() {
                if(this.currentSub<this.form.choiceSubs.length-1) {
                    var element = this.form.choiceSubs[this.currentSub];
                    this.form.choiceSubs.splice(this.currentSub, 1);
                    this.currentSub++;
                    this.form.choiceSubs.splice(this.currentSub, 0, element);
